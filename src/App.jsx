...
:root{
  ...
  --txt:#f0eeff;--mut:#5a5a80;--mut2:#9090b8;--red:#f87171;--gold:#fbbf24;
}
...
.badge{display:inline-flex;align-items:center;gap:8px;background:linear-gradient(135deg,#c084fc12,#2dd4bf12);border:1px solid #c084fc25;border-radius:50px;padding:7px 18px;font-family:'Fira Code',monospace;font-size:11px;letter-spacing:.15em;color:var(--lil2);margin-bottom:24px;}
...
.urow{display:flex;align-items:center;gap:8px;padding:10px 14px;border-bottom:1px solid var(--b);}
...
.titem{display:flex;align-items:center;gap:8px;padding:8px 12px;cursor:pointer;transition:background .1s;border-left:2px solid transparent;}
.titem:hover{background:var(--s2);}
.titem.on{background:var(--s2);border-left-color:var(--lil);}
.tthumb{width:36px;height:36px;border-radius:7px;background:var(--s3);flex-shrink:0;display:flex;align-items:center;justify-content:center;color:var(--mut);font-size:13px;overflow:hidden;}
...
.sp-btn{margin:10px 12px;background:linear-gradient(135deg,var(--teal),var(--teal2));color:#0a0a14;border:none;border-radius:10px;padding:10px;font-family:'Outfit',sans-serif;font-weight:700;font-size:13px;cursor:pointer;width:calc(100% - 24px);}
...
.pb-save{background:linear-gradient(135deg,var(--lil3),var(--mix));color:#fff;border:none;border-radius:8px;padding:7px 18px;font-family:'Outfit',sans-serif;font-weight:700;font-size:13px;cursor:pointer;}
...
.pb-card{background:var(--s1);border:1px solid var(--b);border-radius:13px;padding:14px 16px;}
...
.pb-play{background:linear-gradient(135deg,var(--lil3),var(--mix));color:#fff;border:none;border-radius:7px;width:30px;height:30px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:13px;}
.pb-play:hover{opacity:.85;}
.pb-play.playing{background:linear-gradient(135deg,var(--teal),var(--teal2));color:#0a0a14;}
.pb-del{background:none;border:1px solid var(--b);color:var(--mut);border-radius:7px;width:30px;height:30px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:13px;}
.pb-del:hover{border-color:var(--red);color:var(--red);}
.pb-phon{font-family:'Fira Code',monospace;font-size:10px;color:var(--teal2);margin-bottom:4px;}
...
.err-banner{background:#18080e;border:1px solid #f8717130;color:var(--red);border-radius:10px;padding:10px 13px;font-size:13px;margin-bottom:12px;}
`
    ...
    fetch('https://accounts.spotify.com/api/token', {
      ...
      .then(d => {
        if (d.access_token) {
          ...
          setErr('Erro: ' + (d.error_description || d.error || JSON.stringify(d)))
        }
      })
  ...
  return (
    <>
      ...
      <div className="cb">
        ...
        {err && <button className="btn-back" onClick={() => window.location.href='/'}>Voltar</button>}
      </div>
    </>
  )
}
...
function HomeScreen({ onSelect }) {
  ...
  const phraseCount = (store.get('phrases') || []).length
  return (
    <div className="home">
      <div className="badge"><span className="pulse"/>✦ artanes studio</div>
...
export default function App() {
  ...
  function go(m) { setMode(m); setScreen(m === 'phrasebook' ? 'phrasebook' : 'app') }
  return (
    <>
      <style>{CSS}</style>
      <div className="bg"/>
...
function MusicScreen({ mode, onHome }) {
  ...
  useEffect(() => {
    ...
    fetch(ep[spTab] || ep.saved, { headers: { Authorization: `Bearer ${spToken}` } })
      ...
      .then(d => {
        ...
        setSpTracks(items.filter(t => t && t.id))
      })
      .catch(()=>{})
  ...
  function logoutSpotify() { store.del('sp_token'); window.location.href = '/' }
  ...
  async function loadLyrics(title, artist) {
    ...
    try {
      const txt = await callAI(`Provide complete lyrics for "${title}"${artist?` by "${artist}"`:''}.
Return ONLY valid JSON (no markdown, no explanation):
{"title":"...","artist":"...","language":"English","lyrics":[{"type":"verse","lines":[{"text":"lyric line","phonetic":"fo-NET-ik syllabic","translation":"portuguese translation"}]}]}
Rules: real complete lyrics, min 2 verses + chorus. phonetic = simple syllabic NOT IPA. translation = English to Portuguese. Return ONLY JSON.`)
      ...
      setLyricsErr(e.message || 'Erro ao carregar letra.')
    }
  ...
  async function loadTip(title, artist, lang) {
    ...
    try {
      const txt = await callAI(`Song: "${title}" by ${artist||'?'} (${lang||'English'}). Give ONE English learning tip. Return ONLY JSON (no markdown): {"tag":"Pronunciation|Slang|Phrasal Verbs|Idioms|Culture|Grammar","tip":"Explanation in Portuguese 2-3 sentences","example":"Example line"}`, 300)
  ...
  function handleSearch(e) {
    ...
    pickTrack({id:'q-'+Date.now(),title,artist}); setQuery('')
  }
      ...
      <div className="pt">
        <div className="layout">
          <div className="sidebar">
            <div className="sb-top"><div className="sb-brand"><span className="gl">Art</span><span className="gt">anes</span></div></div>
            <SidebarContent/>
          </div>
          ...
          <div className="content">
            {!selected ? (
              <div className="empty">
                ...
                <div className="empty-sub">{mode==='local'?'Arraste arquivos ou busque pelo nome abaixo':'Escolha uma faixa ao lado ou toque ♪'}</div>
              ...
              <>
                ...
                <div className="np">
                  ...
                  <div>
                    <div className="np-chip">agora visualizando</div>
                    <div className="np-title">{selTitle}</div>
                    <div className="np-artist">{selArtist||'Artista desconhecido'}</div>
                    {selected.file&&<div className="np-file">{selected.file.name}</div>}
                  </div>
                </div>
                <div className="meta">
                  <div className="meta-head">
                    <div className="meta-lbl">✏ editar título / artista</div>
                    <button className="meta-tog" onClick={()=>setShowMeta(v=>!v)}>{showMeta?'fechar':'editar'}</button>
                  </div>
                  {showMeta&&(
                    <>
                      <div className="meta-fields">
                        <div className="mf"><label>Título</label><input value={metaTitle} onChange={e=>setMetaTitle(e.target.value)}/></div>
                        <div className="mf"><label>Artista</label><input value={metaArtist} onChange={e=>setMetaArtist(e.target.value)}/></div>
                      </div>
                      <button className="meta-go" onClick={applyMeta}>Buscar letra →</button>
                    </>
                  )}
                </div>
                {audioUrl&&(
                ...
                {(tipLoading||tip)&&(
                  <div className="tip-box">
                    <div className="tip-head">
                      <div className="tip-title"><span>💡</span><span className="tip-gr">Dica de Inglês</span></div>
                      {tip&&<button className="tip-refresh" disabled={tipLoading} onClick={()=>loadTip(lyrics?.title,lyrics?.artist,lyrics?.language)}>{tipLoading?'…':'↻'}</button>}
                    </div>
...
function PhrasebookScreen({ onHome }) {
  ...
  async function addPhrase() {
    ...
    try {
      const txt = await callAI(`Analyze this text: "${input.trim()}" (language hint: ${lang}).
Return ONLY valid JSON (no markdown):
{"original":"${input.trim()}","language":"${lang}","phonetic":"SIL-AB-ik fo-NET-ik readable pronunciation","translation":"Portuguese translation or meaning","langCode":"en-US"}
langCode options: en-US pt-BR es-ES fr-FR de-DE it-IT ja-JP ko-KR
Return ONLY valid JSON.`, 250)
      const p = parseJSON(txt)
      if (!p) throw new Error('Não foi possível processar. Tente novamente.')
      const newPhrase = {
        id: Date.now().toString(),
        original: p.original || input.trim(),
        phonetic: p.phonetic || '',
        translation: p.translation || '',
        language: p.language || lang,
        langCode: p.langCode || 'en-US',
  ...
  const filtered = search.trim()
    ? phrases.filter(p => p.original.toLowerCase().includes(search.toLowerCase()) || p.translation.toLowerCase().includes(search.toLowerCase()))
    : phrases
      ...
      <div className="pt">
        <div className="pb">
          ...
          <div className="pb-form">
            ...
            {saveErr && <div className="err-banner" style={{marginTop:8}}>⚠ {saveErr}</div>}
            <div className="pb-row">
              <select className="pb-sel" value={lang} onChange={e=>setLang(e.target.value)}>
                {['Inglês','Espanhol','Francês','Italiano','Alemão','Japonês','Coreano','Português','Outro'].map(l=><option key={l}>{l}</option>)}
              </select>
              <button className="pb-save" onClick={addPhrase} disabled={saving||!input.trim()}>
          ...
          {filtered.length === 0 ? (
            <div className="pb-empty">
              ...
              <p style={{color:'var(--mut2)',fontSize:13}}>{phrases.length===0?'Nenhuma frase ainda.\nDigite algo acima para começar!':'Nenhuma frase encontrada.'}</p>
            </div>
          ) : (
                ...
                <div key={p.id} className="pb-card">
                  <div className="pb-ctop">
                    ...
                    <div className="pb-actions">
                      <button className={`pb-play${playingId===p.id?' playing':''}`} onClick={()=>speak(p)}>{playingId===p.id?'⏹':'▶'}</button>
                      <button className="pb-del" onClick={()=>del(p.id)}>🗑</button>
                    </div>
                  </div>
